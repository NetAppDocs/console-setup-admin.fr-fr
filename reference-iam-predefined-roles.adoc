...
| Visionneuse de classification | Permet aux utilisateurs d'afficher les résultats de l'analyse de NetApp Data Classification . Les utilisateurs disposant de ce rôle peuvent afficher les informations de conformité et générer des rapports pour les ressources auxquelles ils sont autorisés à accéder. Ces utilisateurs ne peuvent pas activer ou désactiver l’analyse des volumes, des buckets ou des schémas de base de données. La classification des données ne comporte pas de rôle d'administrateur. 
