...
---
= Déployez manuellement un agent Console dans AWS pour NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Vous pouvez déployer manuellement un agent Console sur un hôte Linux exécutant AWS. Pour déployer manuellement l'agent Console sur votre propre hôte Linux, vous devez vérifier la configuration requise de l'hôte, configurer votre réseau, préparer les autorisations AWS, déployer l'agent Console, puis fournir les autorisations que vous avez préparées.

Le déploiement d'un agent dans AWS lance une instance EC2 exécutant Linux et le logiciel agent Console dans un VPC de votre choix.

.Avant de commencer
* link:concept-agent-install-options.html#agent-cloud-manual-deploy["Découvrez quand déployer manuellement sur AWS"].
...
+
Selon les NetApp data services que vous prévoyez d'utiliser, vous devrez peut-être créer une deuxième stratégie. Pour les régions standard, les autorisations sont réparties sur deux stratégies. Deux stratégies sont nécessaires en raison de la limite de taille maximale des caractères pour les stratégies gérées dans AWS. link:reference-permissions-aws.html["En savoir plus sur les stratégies IAM pour l'agent de console"].
...
Si vous avez des compartiments Amazon S3 dans le même compte AWS que celui où vous avez créé l'agent Console, un système de stockage Amazon S3 apparaîtra automatiquement sur la page *Systèmes*. https://docs.netapp.com/us-en/storage-management-s3-storage/index.html["Apprenez à gérer les buckets S3 depuis la NetApp Console"^]
