...
---
= Déployez un agent de Console sur site à l'aide de VCenter
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Si vous êtes un utilisateur VMWare, vous pouvez utiliser un OVA pour déployer un agent Console dans votre VCenter. Le téléchargement de l'OVA ou l'URL est disponible via la NetApp Console.
...
NetApp prend en charge l'installation de l'agent Console dans votre environnement vCenter. Le fichier OVA inclut une image de machine virtuelle préconfigurée que vous pouvez déployer dans votre environnement VMware. Un téléchargement de fichier ou un déploiement via URL est disponible directement depuis la NetApp Console. Il inclut le logiciel agent Console et un certificat auto-signé.
...
=== Déployez l'agent dans votre environnement vCenter
