...
summary: 'Si vous êtes un utilisateur VMware, vous pouvez utiliser un fichier OVA pour installer un agent Console dans votre vCenter. Le téléchargement du fichier OVA ou l"URL est disponible via la NetApp Console.' 
---
= Installez un agent NetApp Console sur site à l'aide de vCenter
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Si vous êtes un utilisateur VMware, vous pouvez utiliser un fichier OVA pour déployer un agent Console dans votre vCenter. Le téléchargement du fichier OVA ou l'URL est disponible via la NetApp Console.


NOTE: Lorsque vous déployez un agent Console avec vos outils vCenter, vous pouvez utiliser la console web de la machine virtuelle pour effectuer des tâches de maintenance. link:task-agent-vm-config.html["En savoir plus sur la console VM pour l’agent."]
...
NOTE: Vous ne pouvez pas gérer les ressources Google Cloud avec un agent Console installé sur votre infrastructure locale. Pour gérer les ressources Google Cloud, installez un agent dans Google Cloud.
...
.Résultat
Vous devriez maintenant disposer de clés d'accès utilisateur IAM avec les autorisations requises. Après avoir installé l'agent Console, associez ces informations d'identification à l'agent Console depuis la Console.
...
== Installez un agent de console dans votre environnement vCenter

NetApp prend en charge l'installation de l'agent Console dans votre environnement vCenter. Le fichier OVA contient une image de machine virtuelle préconfigurée que vous pouvez déployer dans votre environnement VMware. Un téléchargement de fichier ou un déploiement via une URL est disponible directement depuis la NetApp Console. Il inclut le logiciel agent Console et un certificat auto-signé.
...
. Choisissez de télécharger le fichier OVA ou de copier l'URL à utiliser dans vCenter.




[[deploy-the-agent-in-your-vcenter-environment]]
=== Déployez l'agent dans votre vCenter environnement

Connectez-vous à votre environnement vCenter pour déployer l'agent.

.Étapes
. Téléchargez le certificat auto-signé dans vos certificats de confiance si votre environnement l'exige. Vous remplacez ce certificat après l'installation.
...
+
**** NetApp recommande de laisser cette case décochée afin que l'installation inclue une vérification de configuration de l'agent. La vérification de configuration valide que l'agent a un accès réseau aux points de terminaison requis. Si le déploiement échoue en raison de problèmes de connectivité, vous pouvez accéder au rapport de validation et aux journaux depuis l'hôte de l'agent. Dans certains cas, si vous êtes sûr que l'agent a un accès réseau, vous pouvez choisir d'ignorer la vérification. Par exemple, si vous utilisez encore le link:reference-networking-saas-console-previous.html["points finaux précédents"] utilisé pour les mises à niveau d'agent, la validation échoue avec une erreur. Pour éviter cela, cochez la case pour installer sans vérification de validation. link:reference-networking-saas-console-previous.html#update-endpoint-list["Apprenez à mettre à jour votre liste de points de terminaison"].
