...
summary: NetApp Console propose une solution SaaS pour un accès complet aux fonctionnalités ou une option auto-hébergée pour les organisations confrontées à des restrictions de connectivité. 
---
...
[role="lead"]
NetApp Console propose une solution SaaS pour un accès complet aux fonctionnalités ou une option auto-hébergée pour les organisations confrontées à des restrictions de connectivité.

Vous pouvez utiliser NetApp Console de deux manières : soit comme une solution SaaS entièrement gérée, soit comme une solution auto-hébergée dans votre propre environnement cloud. Avec l'option SaaS, vous accédez à la console via console.netapp.com tandis que NetApp gère toute l'infrastructure, l'authentification et les mises à jour. Vous bénéficiez d'un accès complet aux fonctionnalités directement depuis votre navigateur web. Si vous choisissez l'option auto-hébergée, vous installez un agent Console dans votre environnement AWS, Azure ou Google Cloud. L'interface fonctionne en local et l'agent maintient une connectivité sortante limitée vers NetApp pour les tâches essentielles telles que l'authentification et les mises à jour.

Optez pour la solution SaaS si votre organisation dispose d'une connectivité internet standard. Vous bénéficiez d'une configuration simplifiée, de mises à jour automatiques et d'un accès complet à tous les systèmes de stockage, services de données et fonctionnalités avancées de NetApp . Vous pouvez ajouter des agents directement depuis la console, et NetApp se charge de toute la maintenance.

Choisissez l'option auto-hébergée si votre organisation exige des contrôles stricts en matière de connectivité, de sécurité ou de conformité, comme c'est le cas dans les secteurs gouvernementaux ou réglementés. Vous bénéficiez d'un contrôle accru et d'une meilleure isolation du réseau, mais vous ne pouvez gérer que certains systèmes de stockage NetApp et utiliser des services de données limités, certaines fonctionnalités étant indisponibles. Vous installez manuellement un agent Console et accédez à la Console directement depuis l'interface de l'agent local. Installez votre agent via votre place de marché cloud ou par installation manuelle ; l’agent communique uniquement avec NetApp en sortie, garantissant ainsi l’absence de connexions Internet entrantes.
...
image:diagram-standard-mode.png["Image conceptuelle illustrant l'architecture du mode standard avec l'Internet public où la couche SaaS et l'authentification auth0 sont disponibles."]
...
image:diagram-restricted-mode.png["Image conceptuelle du mode restreint."]
