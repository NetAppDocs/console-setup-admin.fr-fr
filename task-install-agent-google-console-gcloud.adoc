...
summary: 'Pour créer un agent Console dans Google Cloud depuis la NetApp Console ou en utilisant Google Cloud, vous devez configurer votre réseau, préparer les autorisations Google Cloud, activer les API Google Cloud, puis créer l"agent Console.' 
---
= Déployez un agent de Console dans Google Cloud depuis NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Vous pouvez déployer un agent Console dans Google Cloud depuis la Console. Vous devez configurer votre réseau, préparer les autorisations Google Cloud, activer les API Google Cloud, puis déployer l'agent Console.
...
.Avant de commencer
* link:concept-agent-install-options.html#agent-console-deploy["Découvrez quand déployer depuis la console"].
...
NOTE: Lorsque vous déployez un agent dans Google Cloud, celui-ci crée un compartiment pour stocker les fichiers de déploiement.
...
* Un VPC et un sous-réseau répondant aux exigences réseau.
...
** *Étiquettes réseau* : Ajoutez une étiquette réseau à l’instance de l’agent Console si vous utilisez un proxy transparent. Les étiquettes réseau doivent commencer par une lettre minuscule et peuvent contenir des lettres minuscules, des chiffres et des tirets. Les étiquettes doivent se terminer par une lettre minuscule ou un chiffre. Par exemple, vous pouvez utiliser l’étiquette  `console-agent-proxy`.
** *Politique de pare-feu* : Choisissez de créer une nouvelle politique de pare-feu ou de sélectionner une politique de pare-feu existante autorisant les règles entrantes et sortantes requises.
+
