...
---
= Déployez un agent de Console depuis AWS Marketplace
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Vous déployez un agent Console sur AWS directement depuis AWS Marketplace. Pour déployer un agent Console depuis AWS Marketplace, vous devez configurer votre réseau, préparer les autorisations AWS, vérifier les exigences de l'instance, puis déployer l'agent Console.
...
.Avant de commencer
* link:concept-agent-install-options.html#agent-marketplace-deploy["Découvrez quand déployer depuis le AWS Marketplace"].
...
Ouvrez un navigateur Web et accédez à la https://console.netapp.com["NetApp Console"^] pour commencer à utiliser l'agent Console avec la Console.
