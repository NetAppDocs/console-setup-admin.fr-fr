...
---
= Créez un agent NetApp Console à partir de la Place de marché Azure
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
.Avant de commencer
* link:concept-agent-install-options.html#agent-marketplace-deploy["Découvrez quand déployer depuis la Place de marché Azure"].
...
Assurez-vous que l'emplacement réseau où vous prévoyez d'installer l'agent Console répond aux exigences suivantes. Ces exigences permettent à l'agent Console de gérer les ressources de votre cloud hybride.
...
.Résultat
Vous avez maintenant installé l'agent Console et l'avez configuré avec votre organisation Console.
