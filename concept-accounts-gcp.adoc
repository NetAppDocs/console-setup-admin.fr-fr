...
. Lors du déploiement de l'agent de console, vous êtes invité à sélectionner un https://cloud.google.com/iam/docs/service-accounts["compte de service"^] pour l'agent La console obtient des autorisations du compte de service pour créer et gérer les systèmes Cloud Volumes ONTAP , pour gérer les sauvegardes à l'aide de la sauvegarde et de la récupération NetApp , et bien plus encore. Les autorisations sont fournies en associant un rôle personnalisé au compte de service.
