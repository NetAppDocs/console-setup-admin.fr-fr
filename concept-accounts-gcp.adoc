...
---
= Découvrez les projets et les autorisations Google Cloud dans la NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
. Vous devez déployer un agent de console à l’aide d’un compte Google disposant des autorisations nécessaires pour lancer l’agent de console à partir de la console.
. Lors du déploiement de l'agent Console, vous êtes invité à sélectionner un  https://cloud.google.com/iam/docs/service-accounts["compte de service"^] pour l'agent. La Console obtient les autorisations du compte de service pour créer et gérer des systèmes Cloud Volumes ONTAP, gérer les sauvegardes à l'aide de NetApp Backup and Recovery, et plus encore. Les autorisations sont fournies en associant un rôle personnalisé au compte de service.
