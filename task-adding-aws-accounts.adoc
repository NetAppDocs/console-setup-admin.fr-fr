...
[role="lead"]
Ajoutez et gérez vos identifiants AWS pour déployer et gérer des ressources cloud dans vos comptes AWS depuis la NetApp Console. Si vous gérez plusieurs abonnements AWS Marketplace, vous pouvez associer chacun d'eux à des identifiants AWS différents depuis la page Identifiants.
...
link:concept-accounts-aws.html["Découvrez comment la NetApp Console utilise les informations d'identification et les autorisations AWS"].
...
NOTE: Si vous avez déployé un agent de console à partir de la console, les informations d’identification AWS pour le compte dans lequel vous avez déployé un agent de console ont été automatiquement ajoutées. Cela garantit que les autorisations nécessaires sont en place pour la gestion des ressources.
...
Pour fournir à la Console des clés AWS pour un utilisateur IAM, vous devez accorder à cet utilisateur les autorisations requises. La stratégie IAM de la Console définit les actions et ressources AWS que la Console est autorisée à utiliser.
...
NOTE: Les nouvelles informations d’identification de votre fournisseur de cloud peuvent prendre quelques minutes pour être disponibles.
...
Configurez un rôle IAM qui permet à l'application NetApp Console en tant que logiciel en tant que service (SaaS) d'assumer le rôle.
...
----
+
Reportez-vous à link:https://docs.aws.amazon.com/IAM/latest/UserGuide/access_policies-cross-account-resource-access.html["Documentation AWS Identity and Access Management (IAM)"^] pour plus d'informations sur l'accès aux ressources inter-comptes dans IAM.
...
.Résultat
Le rôle IAM dispose désormais des autorisations requises. <<add-the-credentials,Vous pouvez maintenant l'ajouter à la console>>.
...
.Avant de commencer
Si vous venez de créer le rôle IAM, il peut s'écouler quelques minutes avant qu'il soit disponible pour utilisation. Attendez quelques minutes avant d'ajouter les identifiants à la Console.
...
. Sur la page *Informations d’identification de l’organisation*, sélectionnez *Ajouter des informations d’identification* et suivez les étapes de l’assistant.
+
.. *Emplacement des informations d'identification* : sélectionnez *Amazon Web Services > Console*.
...
Pour plus de détails, veuillez vous référer au https://docs.netapp.com/us-en/storage-management-fsx-ontap/requirements/task-setting-up-permissions-fsx.html["Documentation de la console pour Amazon FSx for ONTAP"^]
...
.Avant de commencer
Vous devez créer un agent de console avant de pouvoir configurer un abonnement. link:concept-agent-install-options.html["Apprenez à créer un agent de console"].
...
Lorsque vous vous abonnez sur AWS Marketplace, la dernière étape consiste à associer l'abonnement à votre organisation. Si vous n'avez pas effectué cette étape, vous ne pouvez pas utiliser l'abonnement avec votre organisation.
...
.Étapes
. Confirmez que vous n’avez pas associé votre abonnement à votre organisation Console.
+
...
. Confirmez que l’abonnement est associé à votre organisation.
+
...
.Étapes
. Sélectionnez *Administration > Informations d'identification*.
. Sur la page *Informations d'identification de l'organisation*, sélectionnez le menu d'action pour un ensemble d'informations d'identification, puis sélectionnez *Modifier les informations d'identification*.
...
TIP: Vous ne pouvez pas supprimer les informations d’identification d’un profil d’instance associé à un agent de console.
