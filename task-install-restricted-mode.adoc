...
---
= Déployez l'agent NetApp Console en mode restreint
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
+
link:task-prepare-restricted-mode.html#step-3-review-host-requirements["Examen des exigences des agents"].
...
+
Si vous utilisez plutôt une adresse IP SKU standard, la console utilise l'adresse IP _privée_ de l'agent de la console, au lieu de l'adresse IP publique. Si la machine que vous utilisez pour accéder à la console ne peut pas atteindre l'adresse IP privée, la console ne fonctionne pas.
...
--
Fournissez à la NetApp Console les identifiants du principal de service Azure que vous avez configuré précédemment.
