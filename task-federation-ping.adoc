...
Vous pouvez configurer une connexion fédérée avec PingFederate pour activer l'authentification unique (SSO) pour la Console. La procédure consiste à configurer votre serveur PingFederate pour qu'il fasse confiance à la Console en tant que fournisseur de services, puis à créer la connexion dans la Console.
...
*** Pour , utilisez  `urn:auth0:netapp-cloud-account:<fed-domain-name-pingfederate>` où <fed-domain-name-pingfederate> est le nom de domaine pour la fédération. Par exemple, si votre domaine est  `example.com`, l’ID d’audience/d’entité serait  `urn:auth0:netapp-cloud-account:fed-example-com-pingfederate`.
...
. Créez la connexion avec PingFederate.
+
