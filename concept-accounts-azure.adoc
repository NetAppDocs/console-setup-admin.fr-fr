...
image:diagram_permissions_multiple_azure.png["Image conceptuelle illustrant le compte Azure initial qui reçoit des autorisations via un rôle personnalisé et une identité gérée, ainsi que deux comptes supplémentaires qui reçoivent des autorisations via un rôle personnalisé et un principal de service."]
