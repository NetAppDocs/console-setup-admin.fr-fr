...
Lorsque vous déployez un agent Console depuis la Console, vous devez fournir l'ARN d'un rôle IAM ou les clés d'accès d'un utilisateur IAM. La méthode d'authentification doit disposer des autorisations nécessaires pour déployer l'agent Console dans AWS. Les autorisations requises sont listées dans le link:task-install-agent-aws-console.html#aws-permissions-agent["Politique de déploiement d'agents pour AWS"].

Lorsque la Console lance l'agent Console dans AWS, elle crée un rôle IAM et un profil pour l'agent. Elle associe également une stratégie qui accorde à l'agent Console les autorisations nécessaires pour gérer les ressources et les processus au sein de ce compte AWS. link:reference-permissions-aws.html["Examinez comment l'agent utilise les autorisations"].

image:diagram_permissions_initial_aws.png["Une image conceptuelle qui montre la console déployant l’agent de console dans un compte AWS."]
...
Vous pouvez déployer tous vos systèmes Cloud Volumes ONTAP en utilisant les identifiants AWS initiaux, ou vous pouvez ajouter des identifiants supplémentaires.
...
image:diagram_permissions_multiple_aws.png["Une image conceptuelle qui montre deux comptes supplémentaires. Chacun possède une politique IAM. L'un est associé à un utilisateur IAM et l'autre à un rôle IAM."]
...
Les sections précédentes décrivent la méthode de déploiement recommandée pour l'agent Console, à savoir le déploiement depuis la Console. Vous pouvez également déployer un agent sur AWS depuis AWS Marketplace et installer manuellement le logiciel de l'agent Console sur votre propre hôte Linux ou dans votre vCenter.
