...
[role="lead"]
Pour utiliser les fonctionnalités et services de la NetApp Console , vous devez fournir des autorisations afin que l'agent de la console puisse effectuer des opérations dans votre environnement cloud. Utilisez les liens sur cette page pour accéder rapidement aux autorisations dont vous avez besoin en fonction de votre objectif.
