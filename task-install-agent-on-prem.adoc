...
summary: 'Déployez un agent Console sur site, puis connectez-vous et configurez-le pour qu"il fonctionne avec votre organisation Console.' 
---
= Installez manuellement un agent NetApp Console sur site
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Déployez un agent Console sur site, puis connectez-vous et configurez-le pour qu'il fonctionne avec votre organisation Console.


NOTE: Si vous êtes un utilisateur VMware, vous pouvez utiliser un OVA pour déployer un agent Console dans votre vCenter. link:task-install-agent-on-prem-ova.html["Apprenez-en davantage sur le déploiement d'un agent dans un vCenter."]

Avant l'installation, assurez-vous que votre hôte (machine virtuelle ou hôte Linux) répond aux exigences et que l'agent de la Console dispose d'un accès sortant à Internet ainsi qu'aux réseaux ciblés. Si vous prévoyez d'utiliser les services de données NetApp, ou des options de stockage cloud telles que Cloud Volumes ONTAP, vous devrez créer des identifiants auprès de votre fournisseur de cloud à ajouter à la Console afin que l'agent de la Console puisse effectuer des actions dans le cloud en votre nom.
...
Exécutez l'agent Console sur un hôte x86 conforme aux exigences en matière de système d'exploitation, de RAM et de ports. Assurez-vous que votre hôte répond à ces exigences avant d'installer l'agent Console.
...
Pour fournir des fonctionnalités et des services au sein de la NetApp Console.
 ...
 a| 
https://bluexpinfraprod.eastus2.data.azurecr.io https://bluexpinfraprod.azurecr.io https://occmclientinfragov.azurecr.us (nécessaire pour les déploiements en mode restreint)
 a| 
...
Si vous souhaitez utiliser les services de données NetApp dans AWS ou Azure avec un agent Console local, vous devez configurer les autorisations auprès de votre fournisseur de cloud puis ajouter les informations d'identification à l'agent Console après son installation.
...
Lors de l'installation manuelle d'un agent Console, vous devez préparer votre environnement machine afin qu'il réponde aux exigences. Vous aurez besoin d'une machine Linux et devrez installer Podman ou Docker, selon votre système d'exploitation Linux.
...
** https://mysupport.netapp.com/site/products/all/details/netapp-console/downloads-tab["Site de support NetApp"^] (nécessaire si vous n'avez pas déjà accès à la NetApp Console).
+
...
NOTE: Si vous souhaitez configurer un proxy transparent, vous pouvez le faire après l'installation. link:reference-agent-maint-console-proxy.html["Découvrez la console de maintenance des agents"].
...
--
.Avant de commencer
Si vous venez de créer ces informations d'identification Azure, leur disponibilité peut prendre quelques minutes. Attendez quelques minutes avant d'ajouter les informations d'identification à l'agent Console.
...
.Résultat
L’agent de la console dispose désormais des autorisations nécessaires pour effectuer des actions dans Azure en votre nom.  Vous pouvez désormais accéder à la https://console.netapp.com["NetApp Console"^] pour commencer à utiliser l'agent de console.
