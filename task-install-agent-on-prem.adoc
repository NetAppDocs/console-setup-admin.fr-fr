...
---
= Déployer manuellement un agent Console sur site
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
NOTE: Si vous êtes un utilisateur de VMWare, vous pouvez utiliser un fichier OVA pour déployer un agent Console dans votre vCenter. link:task-install-agent-on-prem-ova.html["Apprenez-en davantage sur le déploiement d'un agent dans un vCenter."]
...
+ * À partir de la version 3.9.47 de l'agent de console, le système prend en charge les points de terminaison répertoriés dans l'option 1.  Les versions précédentes de l’agent de console ne prennent pas en charge la compatibilité descendante.  * L’agent de la console contacte d’abord les points de terminaison dans l’option 2.  Si ces points de terminaison ne sont pas accessibles, il contacte automatiquement les points de terminaison de l'option 1.  * Si vous utilisez l’agent de console avec NetApp Backup and Recovery ou Ransomware Resilience, le système ne prend pas en charge les points de terminaison Option 1.  Autoriser les points de terminaison de l’option 2 et interdire l’option 1.
 ...
 a| 
Si l'agent Console se trouve dans une région AWS Government : \https://*.blob.core.windows.net https://cloudmanagerinfraprod.azurecr.io
 a| 
Pour obtenir les images nécessaires aux mises à niveau de l'agent Console lorsque celui-ci est installé dans une région AWS Government.
 ...
 a| 
Si l'agent Console se trouve dans une région Azure Government : \https://*.blob.core.windows.net https://occmclientinfragov.azurecr.us
 a| 
...
Connectez-vous à la Console et associez l'agent Console à votre organisation. La façon dont vous vous connectez dépend du mode dans lequel vous utilisez la Console. Si vous utilisez la Console en mode standard, vous vous connectez via le site web SaaS. Si vous utilisez la Console en mode restreint ou en mode privé, vous vous connectez localement depuis l'hôte de l'agent Console.
