...
summary: Vous pouvez déployer un agent Console sur AWS directement depuis la NetApp Console. Avant de déployer un agent Console sur AWS depuis la Console, vous devez configurer votre réseau et préparer les autorisations AWS. 
---
= Déployer un agent de Console dans AWS depuis NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Vous pouvez déployer un agent Console sur AWS directement depuis la NetApp Console. Avant de déployer un agent Console sur AWS depuis la Console, vous devez configurer votre réseau et préparer les autorisations AWS.
...
Créez l'agent Console directement depuis la console web Console.
...
. Sur la page *Introduction*, consultez la présentation générale du processus.
. Sur la page *Identifiants AWS*, spécifiez votre région AWS, puis choisissez une méthode d'authentification, soit un rôle IAM que la Console peut assumer, soit une clé d'accès et une clé secrète AWS.
+
...
. Sur la page *Réseau*, spécifiez un VPC, un sous-réseau et une paire de clés pour l'agent, choisissez d'activer ou non une adresse IP publique et spécifiez éventuellement une configuration de proxy.
+
