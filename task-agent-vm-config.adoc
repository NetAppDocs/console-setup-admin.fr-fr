...
. Entrer `1` pour accéder aux outils de diagnostic et suivre les invites à l'écran.  + Par exemple, vous pouvez vérifier que tous les services d’agent sont en cours d’exécution. link:task-troubleshoot-connector.html#check-agent-status["Vérifiez l'état de l'agent de la console"^] .
