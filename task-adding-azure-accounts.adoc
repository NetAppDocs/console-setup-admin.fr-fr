...
image:diagram_azure_authentication.png["Image conceptuelle illustrant comment la console obtient l'authentification et l'autorisation de Microsoft Entra ID avant de pouvoir effectuer un appel API. Le rôle Console définit les autorisations dans Active Directory. Elle est liée à un ou plusieurs abonnements Azure et à un objet principal de service qui représente l'application Console."]
...
.Résultat
Vous pouvez passer à un autre ensemble d'identifiants depuis la page Détails et identifiants https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-deploying-otc-azure.html["lors de l'ajout d'un système à la console"^].
