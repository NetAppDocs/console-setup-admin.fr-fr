...
** link:task-prepare-private-mode.html#step-6-prepare-cloud-permissions["Configurer les autorisations pour le mode privé"]
...
.14+| Créer et gérer des rôles IAM et des profils d'instance pour les instances Cloud Volumes ONTAP | iam:ListInstanceProfiles | Oui | Oui | Non 
...
.6+| Gérer les volumes EBS que Cloud Volumes ONTAP utilise comme stockage backend | ec2:Créer un volume | Oui | Oui | Non 
...
.3+| Activer le chiffrement des données de Cloud Volumes ONTAP à l'aide d'AWS Key Management Service (KMS) | kms:ReEncrypt* | Oui | Non | Non 
...
L'autorisation suivante est désormais requise pour Data Classification :
