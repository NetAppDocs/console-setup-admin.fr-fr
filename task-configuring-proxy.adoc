...
---
= Configurer un agent NetApp Console pour utiliser un serveur proxy
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
IMPORTANT: Une fois le proxy configuré, vous ne pouvez plus en modifier le type. Pour changer de type de proxy, vous devez supprimer l'agent Console et ajouter un nouvel agent avec le nouveau type de proxy.
...
Vous pouvez mettre à jour le serveur proxy transparent existant d'un agent Console en utilisant le `proxy update` commande ou supprimer le serveur proxy transparent en utilisant la `proxy remove` commande.  Pour plus d'informations, consultez la documentation relative àlink:reference-agent-maint-console-proxy.html["Console de maintenance des agents"] .
...
Si vous avez configuré un agent Console pour utiliser un serveur proxy, vous pouvez activer le trafic API direct sur l'agent Console afin d'envoyer les appels API directement aux services du fournisseur de cloud, sans passer par le proxy. Les agents exécutés dans AWS, Azure ou Google Cloud prennent en charge cette option.
