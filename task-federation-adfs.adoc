...
---
= Fédérez NetApp Console avec Active Directory Federation Services
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
Vous pouvez configurer la fédération avec votre serveur AD FS pour activer l'authentification unique (SSO) pour NetApp Console. Le processus implique de configurer votre AD FS pour qu'il fasse confiance à la Console en tant que fournisseur de services, puis de créer la connexion dans NetApp Console.
...
+
.. Créez la relation de confiance à l'aide de PowerShell en exécutant le script suivant :
+
[source, powershell]
----
...
+
*** Lors de la création de l'identificateur de confiance de dépendance, utilisez la valeur *YOUR_TENANT* : `netapp-cloud-account`
