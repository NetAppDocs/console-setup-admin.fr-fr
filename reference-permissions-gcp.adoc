...
---
= Autorisations Google Cloud pour l’agent NetApp Console
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
| config.deployments.get | Non | Oui | Oui 
...
| calculer.disques.créer .8+| Pour créer et gérer des disques sur Cloud Volumes ONTAP. | Oui | Oui | Non 
...
| compute.disks.update | Non | Oui | Non 
...
| cloudbuild.workerpools.get | Pour accéder aux informations du pool de travailleurs lors du déploiement en mode privé et de la conversion des systèmes Cloud Volumes ONTAP à l'aide d'Infrastructure Manager. | Oui | Oui | Oui 


| cloudbuild.workerpools.list | Pour afficher les informations du pool de travailleurs lors du déploiement en mode privé des systèmes Cloud Volumes ONTAP à l'aide d'Infrastructure Manager. | Oui | Non | Non 
...
L' `compute.disks.update`autorisation est ajoutée pour mettre à jour les paramètres IOPS et de débit sur un agrégat ou un disque Hyperdisk Balanced dans les systèmes Cloud Volumes ONTAP.
...
Les  `cloudbuild.workerpools.get` et  `cloudbuild.workerpools.list`autorisations sont ajoutées pour prendre en charge Infrastructure Manager dans les déploiements en mode privé de Cloud Volumes ONTAP dans Google Cloud.
